...
func (c *baseTrafficShapingController) performCheckingForConcurrencyMetric(arg interface{}) *base.TokenResult {
	...
	*initConcurrency = 0
	...
	if specificConcurrency, existed := specificItem[arg]; existed {
		...
		return base.NewTokenResultBlocked(base.BlockTypeFreqParamFlow, fmt.Sprintf("Frequency params traffic shaping controller, current concurrency: %d, specific concurrency: %d", concurrency, specificConcurrency))
	}
	threshold := int64(c.threshold)
	if concurrency <= threshold {
		return nil
	}
	return base.NewTokenResultBlocked(base.BlockTypeFreqParamFlow, fmt.Sprintf("Frequency params traffic shaping controller, current concurrency: %d, threshold: %d", concurrency, threshold))
}
...
func (c *rejectTrafficShapingController) PerformChecking(arg interface{}, acquireCount int64) *base.TokenResult {
	...
	if tokenCount <= 0 {
		return base.NewTokenResultBlocked(base.BlockTypeFreqParamFlow, "rejectTrafficShapingController, the setting tokenCount is <= 0")
	}
	...
	if acquireCount > maxCount {
		...
		return base.NewTokenResultBlocked(base.BlockTypeFreqParamFlow, fmt.Sprintf("rejectTrafficShapingController, the acquired number(%d) is more than max count(%d) of rejectTrafficShapingController", acquireCount, maxCount))
	}
	...
	for {
		...
		if passTime > c.durationInSec*1000 {
			...
			if oldQpsPtr == nil {
				...
				if newQps < 0 {
					return base.NewTokenResultBlocked(base.BlockTypeFreqParamFlow, fmt.Sprintf("rejectTrafficShapingController, the new QPS after subbing acquire(%d) is less than 0.", acquireCount))
				}
			...
			if found {
				...
				if oldRestToken-acquireCount >= 0 {
					...
					return base.NewTokenResultBlocked(base.BlockTypeFreqParamFlow, fmt.Sprintf("rejectTrafficShapingController, the rest token is not enough, oldRestToken: %d, acquire: %d", oldRestToken, acquireCount))
				}
			}
			runtime.Gosched()
		}
...
func (c *throttlingTrafficShapingController) PerformChecking(arg interface{}, acquireCount int64) *base.TokenResult {
	...
	if tokenCount <= 0 {
		return base.NewTokenResultBlocked(base.BlockTypeFreqParamFlow, "throttlingTrafficShapingController, the setting tokenCount is <= 0")
	}
	...
	for {
		...
		if expectedTime <= currentTimeInMs || expectedTime-currentTimeInMs < c.maxQueueingTimeMs {
			...
			return base.NewTokenResultBlocked(base.BlockTypeFreqParamFlow, fmt.Sprintf("throttlingTrafficShapingController, current time(%d) is not reaching to expected time(%d)", currentTimeInMs, expectedTime))
		}
	}
}
