...
func TestFrequencyParamsRulesJsonConverter(t *testing.T) {
	t.Run("TestFrequencyParamsRulesJsonConverter_invalid", func(t *testing.T) {
		properties, err := FrequencyParamsRulesJsonConverter([]byte{'s', 'r', 'c'})
		assert.True(t, properties == nil)
		assert.True(t, err != nil)
	})

	t.Run("TestFrequencyParamsRulesJsonConverter_normal", func(t *testing.T) {
		// Prepare test data
		f, err := os.Open("../../tests/testdata/extension/helper/FreqParamRule.json")
		defer f.Close()
		if err != nil {
			t.Errorf("The rules file is not existed, err:%+v.", err)
		}
